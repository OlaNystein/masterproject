...
class RandomSamplerBase {
 public:
  ...
  enum SampleModeType {
    ...
    kExploredMap, // Get bound online from current explored map boundaries. (Only in kUniform)
...
class RandomSampler {
 public:
  ...
  bool setDistributionParams(Eigen::Vector3d &mean_val, Eigen::Vector3d &std_val);
