...
struct VolumetricGain {
  ...
  // All the hash keys for the associated vertex
  std::vector<std::size_t> unseen_voxel_hash_keys;
  // The hash keys at a particular angle
...
struct Vertex {
  ...
  bool is_hanging;
  // If this vertex is a communication hub
  bool is_comm_hub;
...
struct SerializeVertex {
  ...
  void printit()
  {
    std::cout << "x: " << this->state[0] << " y: " << this->state[1] << " z: " << this->state[2] << std::endl;
...
enum class ConnectStatus {
  ...
  kErrorNoFeasiblePath // Could not find any feasible path from source to target
};
