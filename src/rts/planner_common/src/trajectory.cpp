...
double Trajectory::computeDistanceBetweenTwoTrajectories(const PathType &path_1,
  ...
  if (!interpolatePath(path_1, discrete_length, path_1_intp))
    return dist_ret;

  PathType path_2_intp;
  if (!interpolatePath(path_2, discrete_length, path_2_intp))
    return dist_ret;
...
bool Trajectory::interpolatePath(const TrajectoryType &traj, double discrete_length,
  ...
  for(int i=0; i<(traj_size-1); ++i) {
    ...
    VectorType v2(traj[i+1].position.x, traj[i+1].position.y, traj[i+1].position.z);
    ...
    if(segment_len < 0.01) {
      ...
      if(std::abs( (double)n - (segment_len / discrete_length) ) < 0.01) {
        // Avoid adding traj[i+1] twice (once in this and once in next iteration) if segment length is integral multiple of discrete length
        --n;
      }
      VectorType uvec = vec / segment_len * discrete_length;
      for(int j=0; j<=n; ++j) {
...
double Trajectory::estimateDirectionFromPath(const PathType &path) {
  ...
  for (int i = 2; i < path.size(); ++i) {
    ...
    //                         path[i][2] - path[i - 1][2]);
    // 2
    Eigen::Vector3d cur_dir(path[i][0] - path[0][0],
                            path[i][1] - path[0][1],
                            path[i][2] - path[0][2]);
