...
bool Graph::getPairDistanceMatrix(int** distance_matrix) {
  // int v = boost::num_vertices(graph_);
  // // double D[v][v];
  // int D[10][10];
  // bool success = boost::floyd_warshall_all_pairs_shortest_paths(graph_, D);
  // return success;
  // boost::graph_traits<GraphType>::vertex_iterator 
  //   firstv, lastv, firstv2, lastv2;
  // boost::graph_traits<GraphType>::edge_iterator first, last;

  // std::cout << "GRAPH:" << std::endl;
  // for(boost::tie(firstv, lastv) = boost::vertices(graph_); firstv != lastv; firstv++) {
  //   for(boost::tie(firstv2, lastv2) = boost::vertices(graph_); firstv2 != lastv2; firstv2++) {
  //     std::cout << "(" << getVertexID(*firstv) << " " << getVertexID(*firstv2) << ")";
  //   }
  //   std::cout << std::endl;
  // }
