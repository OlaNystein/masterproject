...
void GraphManager::updateVertexTypeInRange(StateVec& state,
                                      double range) {
...
void GraphManager::convertGraphToMsg(planner_msgs::Graph &graph_msg) {
  ...
  for (Graph::GraphType::edge_iterator it = ei.first; it != ei.second; ++it) {
    ...
    std::tie(edge.source_id, edge.target_id, edge.weight) = graph_->getEdgeProperty(it);
...
void GraphManager::saveGraph(const std::string &path) {
  ...
  wrt_file.close();
  ROS_INFO("Save the graph with [%d] vertices and [%d] edges to a file: %s",
...
void GraphManager::loadGraph(const std::string &path) {
  ...
  // Get the total number of bytes: http://www.cplusplus.com/reference/fstream/ifstream/rdbuf/
  ...
  ROS_INFO("Load the graph with [%d] vertices and [%d] edges from a file: %s",
          getNumVertices(), getNumEdges(), path.c_str());
}

bool GraphManager::getPairDistanceMatrix(Eigen::MatrixXd& distance_matrix) {
  int** D;
  graph_->getPairDistanceMatrix(D);
  return true;
}
