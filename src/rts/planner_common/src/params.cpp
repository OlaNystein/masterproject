...
bool SensorParamsBase::loadParams(std::string ns) {
  ...
  std::cout << ns << " " << rotations[0] << ", " << rotations[1] << ", " << rotations[2] << std::endl;
  ...
  if (type == SensorType::kCamera) {
    ...
    height = h;
    ROS_INFO("Computed multiray_endpoints for volumetric gain [kCamera]: [%d] points.", frustum_endpoints_B.size());
    ...
    height = h;
    ROS_INFO("Computed multiray_endpoints for volumetric gain [kLidar]: [%d] points.", frustum_endpoints_B.size());
  }
...
void SensorParamsBase::getFrustumEndpoints(StateVec &state,
  ...
  ep.clear();
  for (auto &p: frustum_endpoints_B) {
    // auto p_scaled = p * darkness_range;
...
void SensorParamsBase::updateFrustumEndpoints() {
  ROS_INFO("[PARAMS]: fov: h: %f v: %f", fov[0], fov[1]);
  if (type == SensorType::kCamera) {
    // Compute 4 normal vectors for left, right, top, down planes.
    // First, compute 4 coner points.
    // Assume the range is along the hypotenuse of the right angle.
...
void SensorParamsBase::convertBodyToSensor(
  ...
  // Sensor in ROS coordinate, if want to transform to real camera coordinate, apply this TF
...
void BoundedSpaceParams::setBound(Eigen::Vector3d &min_val_in, Eigen::Vector3d& max_val_in) {
  min_val = min_val_in;
  max_val = max_val_in;
}
...
bool PlanningParams::loadParams(std::string ns) {
  ...
  param_name = ns + "/search_sensor_list";
  ros::param::get(param_name, parse_str_list);
  if (parse_str_list.size() <= 0) {
    ROSPARAM_WARN(param_name, "");
  } else {
    search_sensor_list = parse_str_list;
    std::string str_tmp = "Sensors for search: ";
    for (int i = 0; i < search_sensor_list.size(); ++i) {
      str_tmp += search_sensor_list[i] + ", ";
    }
    ROSPARAM_INFO(str_tmp);
  }

  param_name = ns + "/scan_sensor_list";
  ros::param::get(param_name, parse_str_list);
  if (parse_str_list.size() <= 0) {
    ROSPARAM_WARN(param_name, "");
  } else {
    scan_sensor_list = parse_str_list;
    std::string str_tmp = "Sensors for scanning: ";
    for (int i = 0; i < scan_sensor_list.size(); ++i) {
      str_tmp += scan_sensor_list[i] + ", ";
    }
    ROSPARAM_INFO(str_tmp);
  }
  ...
  param_name = ns + "/search_enable";
  if (!ros::param::get(param_name, search_enable)) {
    search_enable = false;
    ROSPARAM_WARN(param_name, search_enable);
  }

  param_name = ns + "/yaw_samples";
  if (!ros::param::get(param_name, yaw_samples)) {
    yaw_samples = 5;
    ROSPARAM_WARN(param_name, yaw_samples);
  }

  param_name = ns + "/switching_threshold";
  if (!ros::param::get(param_name, switching_threshold)) {
    switching_threshold = 0.6;
    ROSPARAM_WARN(param_name, switching_threshold);
  }
  
  param_name = ns + "/search_lambda_multiplier";
  if (!ros::param::get(param_name, search_lambda_multiplier)) {
    search_lambda_multiplier = 1.5;
    ROSPARAM_WARN(param_name, search_lambda_multiplier);
  }

  param_name = ns + "/select_max_yaw";
  if (!ros::param::get(param_name, select_max_yaw)) {
    select_max_yaw = false;
    ROSPARAM_WARN(param_name, select_max_yaw);
  }
  ...
  param_name = ns + "/num_low_gain_steps";
  if (!ros::param::get(param_name, num_low_gain_steps)) {
    num_low_gain_steps = 3;
    ROSPARAM_WARN(param_name, num_low_gain_steps);
  }

  param_name = ns + "/robot_name";
  if (!ros::param::get(param_name, robot_name)) {
    robot_name = "m100";
  ...
  if (!ros::param::get(param_name, proximity_to_waypoint)) {
    proximity_to_waypoint = 1.0;
    ROSPARAM_WARN(param_name, proximity_to_waypoint);
  }

  param_name = ns + "/initial_planning_time";
  if (!ros::param::get(param_name, initial_planning_time)) {
    initial_planning_time = 2.0;
    ROSPARAM_WARN(param_name, initial_planning_time);
  }
...
void PlanningParams::setPlanningMode(PlanningModeType pmode) {
  switch (pmode) {
    ...
    case PlanningModeType::kVerticalExploration:
      ...
      break;
    case PlanningModeType::kVisualSearch:
      type = PlanningModeType::kVisualSearch;
      ROS_WARN("Planning Mode is set to kVisualSearch");
      break;
    default:
      break;
  }
...
bool SearchParams::loadParams(std::string ns) {
  std::string param_name = ns + "/search_behaviour";
  std::string parse_str;
  
  ros::param::get(param_name, parse_str);
  if (!parse_str.compare("kSparse"))
    search_behaviour = SearchBehaviour::kSparse;
  else if (!parse_str.compare("kDense"))
    search_behaviour = SearchBehaviour::kDense;
  else {
    search_behaviour = SearchBehaviour::kSparse;
    ROSPARAM_WARN(ns + "/search_behaviour", "kSparse");
  }

  param_name = ns + "/yaw_per_vert";
  if (!ros::param::get(param_name, yaw_per_vert)) {
    yaw_per_vert = 6;
    ROSPARAM_WARN(param_name, yaw_per_vert);
  }

  param_name = ns + "/num_sets";
  if (!ros::param::get(param_name, num_sets)) {
    num_sets = 80;
    ROSPARAM_WARN(param_name, num_sets);
  }

  param_name = ns + "/max_num_verts_sparse";
  if (!ros::param::get(param_name, max_num_verts_sparse)) {
    max_num_verts_sparse = 25;
    ROSPARAM_WARN(param_name, max_num_verts_sparse);
  }

  param_name = ns + "/max_num_verts_dense";
  if (!ros::param::get(param_name, max_num_verts_dense)) {
    max_num_verts_dense = 30;
    ROSPARAM_WARN(param_name, max_num_verts_dense);
  }

  param_name = ns + "/remove_zero_gain_verts";
  if (!ros::param::get(param_name, remove_zero_gain_verts)) {
    remove_zero_gain_verts = false;
    ROSPARAM_WARN(param_name, remove_zero_gain_verts);
  }

  param_name = ns + "/execute_half_path";
  if (!ros::param::get(param_name, execute_half_path)) {
    execute_half_path = false;
    ROSPARAM_WARN(param_name, execute_half_path);
  }

  param_name = ns + "/sparse_sensor_range";
  if (!ros::param::get(param_name, sparse_sensor_range)) {
    sparse_sensor_range = 7.0;
    ROSPARAM_WARN(param_name, sparse_sensor_range);
  }

  param_name = ns + "/dense_sensor_range";
  if (!ros::param::get(param_name, dense_sensor_range)) {
    dense_sensor_range = 4.0;
    ROSPARAM_WARN(param_name, dense_sensor_range);
  }
...
bool DarpaGateParams::loadParams(std::string ns) {
  ...
  if ((!ros::param::get(param_name, param_val)) || (param_val.size() != 3)) {
    ROSPARAM_WARN(param_name, darpa_frame_offset[0] << "," << darpa_frame_offset[1] << "," << darpa_frame_offset[2]);
  } else {
  ...
  if ((!ros::param::get(param_name, param_val)) || (param_val.size() != 3)) {
    ROSPARAM_WARN(param_name, center[0] << "," << center[1] << "," << center[2]);
  } else {
