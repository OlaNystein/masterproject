...
double RandomSamplerBase::generate(double current_val, double chi_sq_var) {
  ...
  if (sample_mode_ != SampleModeType::kGlobal)
    r += current_val;
...
bool RandomSampler::setDistributionParams(Eigen::Vector3d &mean_val, Eigen::Vector3d &std_val) {
...
bool RandomSampler::setParams(BoundedSpaceParams& global_params,
  ...
  if (rotation) {
    Eigen::Matrix3d rot_W2B;
    rot_W2B = Eigen::AngleAxisd(local_params.rotations[0], Eigen::Vector3d::UnitZ()) *
