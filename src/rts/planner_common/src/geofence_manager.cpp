...
Polygon2d::Polygon2d(const geometry_msgs::Polygon &poly_msg) {
  ...
  // Close the polygon
  bg::append(polygon_.outer(), Point2dType(poly_msg.points[0].x, poly_msg.points[0].y));
  // To enforce the order into clock-wise.
  bg::correct(polygon_);
}

Polygon2d::Polygon2d(Polygon2dType &poly2d){
  bg::assign(polygon_, poly2d);
}
...
bool Polygon2d::doIntersectWithRectangle(const Eigen::Vector2d& center,
  ...
  Polygon2dType rect;
  bg::append(rect.outer(), Point2dType(center.x() + size.x() / 2, center.y() + size.y() / 2));
  bg::append(rect.outer(), Point2dType(center.x() + size.x() / 2, center.y() - size.y() / 2));
  bg::append(rect.outer(), Point2dType(center.x() - size.x() / 2, center.y() - size.y() / 2));
  bg::append(rect.outer(), Point2dType(center.x() - size.x() / 2, center.y() + size.y() / 2));
  bg::append(rect.outer(), Point2dType(center.x() + size.x() / 2, center.y() + size.y() / 2));
...
void GeofenceManager::addGeofenceArea(const geometry_msgs::Polygon poly_msg, bool merge) {
  ROS_WARN("Added new geofence area.");
...
void GeofenceManager::addGeofenceArea(Polygon2d& poly, bool merge) {
  ...
  ROS_WARN("Number of geofence areas: %d", (int)geofence_list_.size());
}
  ...
  while(cont) {
    ...
    for (int i = 0; i < geofence_list_.size(); ++i) {
      ...
      for (int j = i + 1; j < geofence_list_.size(); ++j) {
        if ((geofence_list_[i].polygon.doIntersect(geofence_list_[j].polygon.polygon_))) {
          std::vector<Polygon2d::Polygon2dType> union_poly;
          bg::union_(geofence_list_[i].polygon.polygon_, geofence_list_[j].polygon.polygon_, union_poly);
          if (union_poly.size() == 1) {
            ...
            ROS_WARN("Polygon with inner ring or they don't intersect.");
          }
        }
      }
      if (redo) break;
...
void GeofenceManager::addGeofenceAreas(std::vector<Polygon2d>& poly_list, bool merge) {
  for (auto &p : poly_list) {
    addGeofenceArea(p, merge);
  }  
}

void GeofenceManager::getLocalGeofenceAreas(Eigen::Vector2d &current_position, double radius,
...
GeofenceManager::CoordinateStatus GeofenceManager::getPathStatus(
  ...
  Eigen::Vector2d p1(start.x() + box_size.x() / 2.0, start.y() + box_size.y() / 2.0);
  Eigen::Vector2d p2(start.x() + box_size.x() / 2.0, start.y() - box_size.y() / 2.0);
  Eigen::Vector2d p3(start.x() - box_size.x() / 2.0, start.y() - box_size.y() / 2.0);
  Eigen::Vector2d p4(start.x() - box_size.x() / 2.0, start.y() + box_size.y() / 2.0);

  Eigen::Vector2d p_dir = end - start;
  Eigen::Vector2d p1_end; p1_end = p1 + p_dir;
  Eigen::Vector2d p2_end; p2_end = p2 + p_dir;
  Eigen::Vector2d p3_end; p3_end = p3 + p_dir;
  Eigen::Vector2d p4_end; p4_end = p4 + p_dir;
  ...
  if ((orientation >= M_PI_2) && (orientation <= M_PI)) {
    ...
    bg::append(path_polygon.outer(), Polygon2d::Point2dType(p3_end.x(), p3_end.y()));
    bg::append(path_polygon.outer(), Polygon2d::Point2dType(p4_end.x(), p4_end.y()));
    bg::append(path_polygon.outer(), Polygon2d::Point2dType(p1_end.x(), p1_end.y()));
    bg::append(path_polygon.outer(), Polygon2d::Point2dType(p1.x(), p1.y()));
    ...
    bg::append(path_polygon.outer(), Polygon2d::Point2dType(p4_end.x(), p4_end.y()));
    bg::append(path_polygon.outer(), Polygon2d::Point2dType(p1_end.x(), p1_end.y()));
    bg::append(path_polygon.outer(), Polygon2d::Point2dType(p2_end.x(), p2_end.y()));
    bg::append(path_polygon.outer(), Polygon2d::Point2dType(p2.x(), p2.y()));
...
bool GeofenceManager::loadParams(std::string ns) {
  ...
  if (!ros::param::get(param_name, geofence_init_list)) {
    ...
    for (auto &s: geofence_init_list) {
      ...
      points.push_back(Eigen::Vector2d(g_center.x() + g_size.x() / 2, g_center.y() + g_size.y() / 2));
      points.push_back(Eigen::Vector2d(g_center.x() + g_size.x() / 2, g_center.y() - g_size.y() / 2));
      points.push_back(Eigen::Vector2d(g_center.x() - g_size.x() / 2, g_center.y() - g_size.y() / 2));
      points.push_back(Eigen::Vector2d(g_center.x() - g_size.x() / 2, g_center.y() + g_size.y() / 2));
